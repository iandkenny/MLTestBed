...
public class FDRPSO extends ClassicPSO
{
	 ...
	 * 
	 */
	public FDRPSO(Topology neighbourhood, Properties params, Properties runparams)
			throws Exception
	{
...
//					+ ((c3 * rnd.nextDouble()) * (calcNBest(i, particle) - p));
			...
			, ((c3 * rnd.nextDouble()) * subtract(calcNBest(i, particle) , p)));
...
//			p = posAdjust(i, p);
	...
	private double calcNBest(int i, Particle particle)
	{
		...
		double scoreX = testFunction.Objective(particle);
		for (int j = 0; j < mSwarm.size(); j++)
		{
			Particle cur = mSwarm.get(j);
			double scoreCur = testFunction.Objective(cur);
			...
			double nScore = (scoreCur - scoreX)/ Math.abs(cur.getPosition().get(i)-particle.getPosition().get(i)); 
	...
	{
		super.init();
		maxw = w = Double.valueOf(params.getProperty("w","0.729")).doubleValue();
...
//			VMax = Double.valueOf(params.getProperty("VMax")).doubleValue();
			minw = Double.valueOf(params.getProperty("minw", "0")).doubleValue();
