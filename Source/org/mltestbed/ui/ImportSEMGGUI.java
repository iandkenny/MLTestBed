...
import org.mltestbed.data.rawimport.ImportSEMGDB1Data;

public class ImportSEMGGUI extends JDialog
		implements
			ActionListener,
			PropertyChangeListener
{
	...
	private static final String IMPORT_SEMG_DATA = "Import SEMG Data";
	...
	private ImportSEMGDB1Data ing[];

	public ImportSEMGGUI(Properties prop)
	{
		super();
		this.prop = prop;
		if (this.prop == null)
		...
		// initProps();
		// saveProps();
		connectString = this.prop
				.getProperty("sourceConnectString", "SEMGData");
		createGUI();
		// openDB();
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			ing = new ImportSEMGDB1Data[MAX_THREADS];
			ok.setEnabled(!ok.isEnabled());
			progressbar.setIndeterminate(true);
			for (int i = 0; i < MAX_THREADS; i++)
			{
				ing[i] = new ImportSEMGDB1Data(progressbar, prop);
				ing[i].start();
			}
