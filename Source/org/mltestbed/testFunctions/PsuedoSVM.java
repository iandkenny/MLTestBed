...
public class PsuedoSVM extends TestBase
{
	...
	private static final String USE_ABSOLUTE_VALUE = "UseAbsoluteValue";
	private static final String USE_LOG_VECTOR = "useLogVector";
	private static final String USE_EXP = "useExp";
	...
	private boolean expVec = false;
	protected double[] logvector = null;
	protected double[] expvector = null;
	protected double[][] matrix = null;
	private boolean useAbs = false;
	private boolean powVec = false;
	protected double[] powvector = null;
	...
	protected double CosmologyObjective(Particle p, ArrayList<Double> row)
	{
		...
		mResult = calcError(required, sum);
	...
	protected double CosmologyObjective3(Particle p, ArrayList<Double> row)
	{
		...
		mResult = calcError(required, score);
	...
	protected void createParams()
	{
		...
		setLogVec(false);
		params.setProperty(USE_EXP, "false");
		setExpVec(false);
		params.setProperty(USE_REVELANCE_VECTOR, "false");
		setRelVec(false);
		params.setProperty(USE_ABSOLUTE_VALUE, "false");
		setUseAbs(false);
	}
	/**
	 * @return the expVec
	 */
	public boolean isExpVec()
	{
		return expVec;
	}
	/**
	 * @param expVec the expVec to set
	 */
	public void setExpVec(boolean expVec)
	{
		this.expVec = expVec;
	}
	 ...
	 * @throws Exception
	 */
	protected Vector<Double> genericCalc(Vector<Double> v, Vector<Double> input)
			throws Exception
	{
		Vector<Double> res = new Vector<Double>();
		double A[][];
	...
	protected double genericObjective(Particle p, ArrayList<Double> row)
	{
		Vector<Double> v = p.getPosition();
		Vector<Double> inpVec = new Vector<Double>(row);
		Vector<Double> vec = new Vector<Double>(v);
		...
		p.setFuncSpecific("<Output><Input>" + inpVec.toString()
				+ "</Input><RequiredValue>" + required
				+ "</RequiredValue><SumResult>" + sum + "</SumResult><Predict>" + Math.round(sum) + "</Predict><Result>"
				+ mResult + "</Result></Output>");
		return super.Objective(p);
	}
	/**
	...
	protected double heartObjective(Particle p, ArrayList<Double> row)
	{
		...
		p.setFuncSpecific("<Output><Input>" + inpVec.toString()
				...
				+ "</SumResult><Result>" + mResult + "</Result></Output>");
		return super.Objective(p);
	}
			...
			ArrayList<Double> next)
	{
		...
		for (int i = 0; i < result.size(); i++)
		{
			sum += calcError(result.get(i), next.get(i));
		}
		// Absolute is Equivalent??
		// mResult = calcError((required - sum), 2));
		// mResult = Math.abs(required - sum);
		// mResult = required - sum;
		mResult = sum;
	...
	public void init()
	{
		...
		buf = params.getProperty(USE_ABSOLUTE_VALUE, "false");
		setUseAbs(buf.equalsIgnoreCase("true"));
	}
	/**
	 * @return the logVec
	 */
	...
	protected double PDObjective(Particle p, ArrayList<Double> row)
	{
		...
		p.setFuncSpecific("<Output><Input>" + inpVec.toString()
				...
				+ "</SumResult><Result>" + calcError(required,
						((sum > 2.0) ? 2.0 : (sum < 1.0) ? 1.0 : sum))
	...
	{
		boolean flag = true;
		double sum = Math.sqrt(Math.abs(v.get(0) + 1.e-7)); // make sure its not
															// zero
...
//		int vecSize = factor;
		int vecSize = inputs;
		int add = (subVec) ? vecSize : 0;
		add += (powVec) ? vecSize  : 0;
		add += (logVec) ? vecSize  : 0;
